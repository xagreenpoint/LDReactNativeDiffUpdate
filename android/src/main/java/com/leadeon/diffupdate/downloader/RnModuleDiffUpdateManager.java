...
public class RnModuleDiffUpdateManager {
    ...
    private void checkVersion() {
        ...
        versionReq.setAppKey("23723021a0f92fb7bea8074a03222fb3");
        versionReq.setRnVersion("1.0");
        versionReq.setAppVersion("1.1.0");
        versionReq.setPlatform("android");
        versionReq.setReqBody(versions);

        OkHttpManager.getInstance().post("http://192.168.10.120:8088/bsDiff/RnPackageUpdate/getUpdateList", versionReq, new OkHttpManager.DataCallBack() {
            @Override
            public void requestSuccess(String result) {
                ...
                if (versionRes != null && versionRes.getRetCode().equals("000000")) {
                    //表示业务成功
                    List<RnCheckRes> list = versionRes.getRspBody().getPatchs();
